 ...
 */
export function calculateAverageIntensityScore(
...
export function detectSpike(weeklyLoads: number[]): SpikeResult {
  ...
  const avgPrevious =
    previousWeeks.length > 0
      ? previousWeeks.reduce((sum, load) => sum + load, 0) /
...
export function computeConsistency(
  ...
  const activeWeeks = weeklyAggregates.filter(
    (week) => week.trainingCount >= minTrainingsPerWeek,
  ).length;
...
export function computeTrend(
  ...
  const previousAvg =
    previous.reduce((sum, load) => sum + load, 0) / previous.length;
...
export function calculateWeeklyTimeSeries(trainings: TrainingDTO[]): Array<{
  ...
  Array.from(weeksMap.entries())
    ...
    .forEach(([, weekTrainings]) => {
      ...
      const minutes = weekTrainings.reduce(
        (sum: number, t: TrainingDTO) => sum + t.durationMinutes,
        0,
      );
      const load = weekTrainings.reduce(
        (sum: number, t: TrainingDTO) => sum + calculateTrainingLoad(t),
        0,
      );
 ...
 */
export function getTopTrainingsByLoad(
  trainings: TrainingDTO[],
  limit: number = 5,
): TrainingDTO[] {
  return [...trainings]
